# Documentation Index (EN)

English documentation for Web Security Scenario Lab. Russian mirror: [`../ru/index.md`](../ru/index.md).

> New documents are added here and mirrored in `docs/ru/`. Several core documents still live in
> the repository root and will be migrated into `docs/` incrementally; links below point to their
> current location.

## Overview
- [README](../../README.md) — project overview and positioning
- [Architecture](../../ARCHITECTURE.md) — directory model, engine, evaluation, tooling
- [Roadmap](../../ROADMAP.md) — staged delivery plan

## Scenario model
- [Scenario Dynamics](../../SCENARIO-DYNAMICS.md) — randomization, seeds, mutation rules
- [Evaluation Outputs](../../EVALUATION-OUTPUTS.md) — ground-truth fields, `detectionExamples`

## Modules
- [Visual Manipulation](../../MODULES-VISUAL-MANIPULATION.md)
- [Link & Domain Security](../../MODULES-LINK-DOMAIN-SECURITY.md)

## Platform & policy
- [Browser Compatibility](../../BROWSER-COMPATIBILITY.md)
- [I18N](../../I18N.md)
- [Security & Deployment](../../SECURITY-DEPLOYMENT.md)
- [Contributing](../../CONTRIBUTING.md) · [DCO](../../DCO.md)

## Native to `docs/`
- [Adding a Framework or Version](./adding-frameworks.md) — Phase 3: framework robustness pages, the
  coverage matrix, install strategies (root alias vs nested node_modules), and the verify checklist
